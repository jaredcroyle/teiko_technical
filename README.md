...
Bob’s first question is:

> *“What is the frequency of each cell type in each sample?”*

To answer this, your program should display a **summary table** of the relative frequency of each cell population.

For each sample:

- Calculate the **total number of cells** by summing the counts across all five populations.
- Compute the **relative frequency** of each population as a percentage of the total cell count for that sample.

Each row in the summary table represents one population from one sample and should have the following columns:
...
- **Compare the differences** in cell population relative frequencies of melanoma patients receiving miraclib who respond (*responders*) versus those who do not (*non-responders*), with the overarching aim of predicting response to the treatment miraclib.
...
- **Report which cell populations** have a significant difference in relative frequencies between responders and non-responders.
  - Include appropriate **statistical tests** and clearly state which differences are significant to support any conclusion and convince Yah of Bob’s findings.
